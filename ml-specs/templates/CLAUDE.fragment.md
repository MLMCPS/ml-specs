...
4. **`docs/GLOSSARY.md`** — when a term in a spec or a diff is ambiguous. What a word means *here*
   and what it is not to be confused with. **Optional**: absent is a valid state, not a gap.
5. **`docs/CONTEXT.md`** — when you need the *why*: what the project is for, who uses it, and the
   constraints that are not derivable from the code. **Optional**, same as above. This file holds
   what `CLAUDE.md` must not inline — `CLAUDE.md` points, `CONTEXT.md` explains, and neither
   copies the other.
6. **`docs/SKILLS.md`** — the third-party catalog skills matching this stack. A pointer list, not
...
7. **Agentic retrieval last** — `Explore` / grep / glob, for exact lines only.
